...
export default function LeaveControl() {
  ...
  const [activeCategory, setActiveCategory] = useState<'leave' | 'vacate'>('leave');
  ...
  const filteredLeaves = leaves.filter((leave: any) => {
    ...
    if (activeCategory === 'leave' && isVacating) return false;
    if (activeCategory === 'vacate' && !isVacating) return false;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Application Category Toggle Bar */}
      <div className="flex items-center gap-2 bg-slate-100/80 p-1.5 rounded-2xl w-fit border border-slate-200">
        ...
        >
          Outpass & Leave Applications
        </button>
        <button
          onClick={() => setActiveCategory('vacate')}
          className={`px-5 py-2 rounded-xl text-xs font-black transition-all cursor-pointer ${
            activeCategory === 'vacate'
              ? 'bg-indigo-600 text-white shadow-md'
              : 'text-slate-600 hover:text-slate-900'
          }`}
        >
          Permanent Hostel Exit & Vacating Applications
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center bg-slate-50 border border-dashed border-slate-200 rounded-2xl m-4">
            ...
            <p className="text-sm font-bold text-slate-600">No {activeCategory === 'leave' ? 'outpass & leave' : 'vacating & exit'} applications found</p>
            <p className="text-xs text-slate-400 mt-1">Submitted requests will appear here for review.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead className="bg-slate-100/70 text-slate-600 font-bold uppercase text-[10px] tracking-wider border-b border-slate-200">
                <tr>
                  ...
                  <th className="p-4">{activeCategory === 'leave' ? 'Leave Type & Reason' : 'Vacating Reason'}</th>
                  <th className="p-4">{activeCategory === 'leave' ? 'Duration & Dates' : 'Vacating Date'}</th>
                  <th className="p-4">{activeCategory === 'leave' ? 'Destination & Contacts' : 'Refund Bank & Signature'}</th>
                  ...
                  return (
                    <tr key={leave.id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="p-4 max-w-xs">
                        <span className={`inline-block px-2.5 py-0.5 border rounded-md text-[10px] font-black uppercase tracking-wider mb-1 ${
                          leave.leaveType?.includes('Vacat') || leave.leaveType?.includes('Exit')
                      ...
                      {/* Destination & Contacts / Vacating Bank Details */}
                      <td className="p-4 space-y-1 text-[11px]">
                        {leave.bankName ? (
                          <div className="bg-purple-50/80 border border-purple-200/80 p-2 rounded-lg space-y-0.5">
                            <p className="text-[10px] font-black text-purple-900 uppercase">Refund Bank Details:</p>
                            <p className="font-bold text-slate-800">{leave.bankName} • A/C: {leave.accountNumber}</p>
                            <p className="text-[10px] text-slate-600">IFSC: {leave.ifscCode} | Holder: {leave.accountHolder || leave.studentName}</p>
                            {leave.depositAmount && <p className="text-[10px] font-extrabold text-emerald-700">Deposit Refund: ₹{leave.depositAmount}</p>}
