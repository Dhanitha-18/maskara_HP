...
export default function MessMenuControl() {
  ...
  const addMealSlot = (key: string) => {
    setCmsData((prev: any) => {
      const copy = { ...prev };
      if (!copy.menu[activeDay]) copy.menu[activeDay] = {};
      copy.menu[activeDay][key] = {
        name: `New ${key} Dish`,
  ...
  const startEditPolicy = () => {
    if (!cmsData) return;
    setPolicyTitle(cmsData.policy?.title || '');
    setPolicyPoints([...(cmsData.policy?.points || [])]);
  ...
  const savePolicy = () => {
    setCmsData((prev: any) => ({
      ...prev,
      policy: { title: policyTitle, points: policyPoints.filter(p => p.trim() !== '') }
    }));
    setEditingPolicy(false);
    toast.success('Policy guidelines updated locally.');
  };

  // Supplier Notes
  const startEditSupplier = () => {
    if (!cmsData) return;
    setSupplierTitle(cmsData.supplierNotes?.title || '');
    setSupplierPoints([...(cmsData.supplierNotes?.points || [])]);
  ...
  const saveSupplier = () => {
    setCmsData((prev: any) => ({
      ...prev,
      supplierNotes: { title: supplierTitle, points: supplierPoints.filter(p => p.trim() !== '') }
    }));
    setEditingSupplier(false);
    toast.success('Dessert & Supplier notes updated locally.');
  };
  ...
  const saveInclusion = (key: string) => {
    ...
    toast.success(`Inclusion card for ${key} updated locally.`);
  };
  ...
  const saveTableCell = () => {
    ...
    setCmsData((prev: any) => {
      const copy = { ...prev };
      if (!copy.menu[day]) copy.menu[day] = {};
      if (!copy.menu[day][meal]) {
        copy.menu[day][meal] = {
          name: '',
      ...
      copy.menu[day][meal].name = tableMealName;
      return copy;
    });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-16 relative">
      ...
      {/* Hero Banner CMS */}
      <div className="relative w-full h-[240px] sm:h-[300px] overflow-hidden rounded-3xl shadow-xl border border-slate-200 group">
        <img 
          src={MESS_HERO_IMAGE} 
          alt="Mess Hero" 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/40 to-transparent flex flex-col justify-end p-6 sm:p-10">
          <div className="max-w-7xl w-full mx-auto flex justify-between items-end gap-6">
            <div className="flex-1">
              {!editingHeader ? (
                <>
                  <span className="text-[10px] sm:text-xs text-amber-400 font-black tracking-widest uppercase block mb-1">
                    {cmsData.header?.badge || 'WEF 2026 • Official Menu'}
                  </span>
                  <h1 className="text-xl sm:text-3xl font-black text-white tracking-tight uppercase leading-tight drop-shadow-md">
                    {cmsData.header?.title || 'Mess & Dining Portal'}
                  </h1>
                  <p className="text-xs text-slate-200 mt-1 font-semibold tracking-wide uppercase">
                    {cmsData.header?.subtitle || 'Hygienic, nutritionally balanced food menu & guest dining management'}
                  </p>
                </>
              ) : (
                <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-700/80 space-y-3 max-w-2xl text-xs">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] uppercase text-slate-400 font-bold mb-1">Badge Text</label>
                      <input 
                        type="text" 
                        value={headerBadge}
                    ...
                    <div>
                      <label className="block text-[10px] uppercase text-slate-400 font-bold mb-1">Portal Title</label>
                      <input 
                        type="text" 
                        value={headerTitle}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase text-slate-400 font-bold mb-1">Portal Subtitle</label>
                    <input 
                      type="text" 
                      value={headerSubtitle}
                      onChange={e => setHeaderSubtitle(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-700 text-white rounded p-1.5 font-semibold outline-none"
                    />
                  </div>
                  <div className="flex gap-2 justify-end pt-1">
                    <button onClick={() => setEditingHeader(false)} className="px-3 py-1 bg-slate-800 text-slate-300 font-bold rounded">Cancel</button>
                    <button onClick={saveHeader} className="px-3 py-1 bg-indigo-600 text-white font-bold rounded">Apply</button>
                  </div>
                </div>
              )}
            </div>
            ...
            {!editingHeader && (
              <button 
                ...
                className="opacity-0 group-hover:opacity-100 bg-white/25 hover:bg-white/40 border border-white/40 text-white px-3.5 py-1.5 rounded-xl text-xs font-black uppercase flex items-center gap-1.5 transition-all shadow-md cursor-pointer mb-2"
              >
